...
Методы класса:
    ...
    `saveProduct(id: string)` - сохранение товара для подробного отображения
    `getSavedProduct()` - получение товара для подробного отображения
...
Методы класса:
    ...
    `setProduct(product: IProduct)` - добавление товара, который был получен в параметре, в массив корзины
...
Методы класса:
    `saveData()` - сохра`нение данных в модели (c возможностью сохранить только одно значение)
    `getData()` - получение все данных покупателя
